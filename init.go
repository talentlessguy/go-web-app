...
func setupGlue() {
	...
	templateHTML, err := os.Create("template.html")

	if err != nil {
		log.Fatal("Failed to create template.html")
	}

	defer templateHTML.Close()

	// %PORT% and %IS_PRODUCTION% will be replaced when running build or dev
	templateHTML.WriteString(`<!DOCTYPE html>
	<html lang="en">
	<head>
		<meta charset="UTF-8">
	...
	<body>
		...
		<script>
			...
			const isInProduction = %IS_PRODUCTION%
          ...
          conn.onmessage = evt => location.reload()
				...
				try {
					if (window.WebSocket) {
						tryConnectToReload('ws://localhost:%PORT%/reload')
					} else {
